---
category: URL
flag: Possibly compromised
country:
  - Chad/Tchad
attack_category: Phishing
other_related_domains: "[[itsolution4india.com]]"
date_added: 2025-11-20
urlscan_subdomains:
  - apex303.diplomatie.gouv.td
  - gtr303.diplomatie.gouv.td
  - ktm303.diplomatie.gouv.td
  - powergaming88.diplomatie.gouv.td
---
## **Summary**
- Domain belongs to the Government of Chad/Tchad
- Domain points to an active website
- Website is based on the Wordpress Platform and is built using the Elementor page builder
- Website is also using other plugins like News Element and Google Translate